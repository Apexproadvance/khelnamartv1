...
import type { ProductCatalog, WishlistItem } from '@/lib/types';
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data } = await supabase
        .from('wishlist_items')
        .select('*, product_catalog(*, product_images(*), categories(*), products(*, sellers(*)))')
        .eq('user_id', user!.id)
        .order('created_at', { ascending: false });
      setItems((data as WishlistItem[])?.map((w) => w.product_catalog).filter((p): p is ProductCatalog => p != null) ?? []);
      setLoading(false);
    }
    load();
  }, [user, navigate]);
