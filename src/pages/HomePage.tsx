  ...
  useEffect(() => {
    async function load() {
      const [featRes, newRes, trendRes, catRes, featuredListingRes] = await Promise.all([
        supabase.from('product_catalog').select('*, product_images(*), categories(*), products(*, sellers(*))').eq('is_featured', true).eq('is_active', true).order('rating', { ascending: false }).limit(8),
        supabase.from('product_catalog').select('*, product_images(*), categories(*), products(*, sellers(*))').eq('is_active', true).order('created_at', { ascending: false }).limit(4),
        supabase.from('product_catalog').select('*, product_images(*), categories(*), products(*, sellers(*))').eq('is_active', true).order('rating_count', { ascending: false }).limit(4),
      ...
      const baseFeatured = featRes.data as ProductCatalog[] ?? [];
