...
import { Package, CheckCircle2, Clock, Truck, MapPin, Check, X } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  async function cancelOrder(orderId: string) {
    ...
    await supabase.from('order_items').update({ status: 'cancelled' }).eq('order_id', orderId).in('status', ['pending', 'accepted']);
    setOrders(orders.map((o) => o.id === orderId ? { ...o, status: 'cancelled', order_items: o.order_items?.map((oi) => oi.status === 'pending' || oi.status === 'accepted' ? { ...oi, status: 'cancelled' } : oi) } : o));
                ...
                <div className="p-4">
                  ...
                  {(order.status === 'pending' || order.status === 'confirmed') && (
                    <button
                      ...
                      className="mt-4 flex items-center gap-1.5 rounded-lg border border-error-200 px-3 py-1.5 text-sm text-error-600 transition-colors hover:bg-error-50 disabled:opacity-50"
                    >
