...
import type { CartItem, SellerOffer } from '@/lib/types';

interface CartContextValue {
  items: CartItem[];
  loading: boolean;
  count: number;
  addToCart: (product: SellerOffer, quantity?: number) => Promise<void>;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  async function refresh() {
    ...
    const { data } = await supabase
      .from('cart_items')
      .select('*, products(*, sellers(*), product_catalog(*, product_images(*)))')
      .eq('user_id', user.id)
  ...
  async function addToCart(product: SellerOffer, quantity = 1) {
    if (!user) return;
